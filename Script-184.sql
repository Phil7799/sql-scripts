...
select ac.Corporate as COPS , monthname(ct.dated_) as month_, year(ct.dated_)as year_, sum(ct.Rev_) 
from combinedata ct
join all_corporates.all_corporates ac on ct.CORPORATE_ID = ac.CorporateID
where year(ac.CreatedOn) >= '2021' and year(ac.CreatedOn) <= '2022'
group by ac.Corporate , monthname(ct.dated_),year(ct.dated_);